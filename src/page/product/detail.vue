<template>
  <div class="goods">
    ...
    <van-swipe class="goods-swipe" :autoplay="3000">
      <van-swipe-item v-for="thumb in goods.gpic_small" :key="thumb">
        <img :src="thumb" >
      </van-swipe-item>
    </van-swipe>
    ...
    <van-cell-group>
      <van-cell>
        <span class="goods-price">{{ formatPrice(goods.gprice) }}</span>
        <span class="goods-market-price">{{ formatPrice(goods.gprice) }}</span>
        <div class="goods-title">{{ goods.gname }}</div>
        <div class="goods-subtit">{{goods.gtext}}</div>
      </van-cell>
...
<!--     
    <van-cell-group class="goods-cell-group">
      <van-cell  is-link @click="showSku" >
        <template slot="title">
            <span style="margin-right: 10px;">数量选择</span>
            <span >10件装</span>
        </template>
      </van-cell>
...
export default {
  ...
  methods: {
    ...
    let id = this.$route.params.id
     getGoodDetail(id).then(Response=>{
       let data = Response.data
       this.goods = data
       skuData.goods_id = data.gid
       skuData.sku.price = data.gprice
       skuData.goods_info.title = data.gname
       skuData.sku.stock_num = data.gstore
       skuData.goods_info.picture = data.gpic_small[0]
       this.skuData = skuData
     })
    },
    formatPrice(data) {
    ...
    onBuyClicked(data) {
      var g = {
        desc: this.goods.gtext,
        id:this.goods.gid,
        imageTag: "比加入时降5元",
        imageURL: this.goods.gpic_small,
        price: this.goods.gprice,
        quantity:data.selectedNum,
        title: this.goods.gname
      }
      var goods = []
      goods.push(g)
      this.$router.push({
    ...
    onAddCartClicked(data) {
      var join_cart = {
        cgoods_id:data.goodsId,
        cnum:data.selectedNum
      }
      AddCart(JSON.stringify(join_cart)).then(Response=>{
         this.$toast("加入购物车成功")
      })
