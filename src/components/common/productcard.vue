<template>
    <van-cell-group class="additional">
        ...
        >
            <template slot="title">
              <span @click="cardClict(product)">{{product.title}}</span>
                </template>
            <template slot="thumb">
                <img :src="product.imageURL"  @click="cardClict(product)"/>
            ...
            <template slot="tags">
                ...
                <van-stepper v-if="iscard" v-model="product.quantity" :max="product.max"  :min="product.min" @change="stepperChange(1)"/>
            </template>
        </van-card>
